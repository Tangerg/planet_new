...
Detail lookups cross Wails as explicit `found` / `notFound` results; scans cross as `cancelled` / `partial` / `complete`, while a missing desktop bridge is `unavailable` in the frontend contract. Errors retain their Go cause internally but cross Wails only as stable `code + operation` data (`invalidArgument`, `notFound`, `unavailable`, `incomplete`, `cancelled`, `failed`), carried on the rejection's structured `cause` through the bound service's error marshaller — the frontend reads that payload and never parses the human-readable message. Anything the adapter did not classify falls through to Wails' default marshalling, so the frontend fails it closed rather than trusting an unknown shape. Local entity ids are parsed through one domain rule at every trust boundary: malformed Wails arguments fail as `invalidArgument`, corrupt SQLite rows fail hydration, and invalid media paths are rejected before catalog access. Every bound method that reaches a use case takes a `context.Context` first, which Wails recognises and keeps out of the generated TypeScript: each call runs under its own context, cancelled when the frontend abandons the call. The adapter joins that to a lifetime owned by `App`, which the composition root ends before tearing infrastructure down, so a scan caught by a quit is cancelled rather than abandoned mid-transaction. From there the context is propagated through application, scanner, SQLite and outbound HTTP work. An incomplete filesystem observation may upsert readable tracks but can never prune unseen rows; only a complete `ScanSnapshot` has deletion authority. Scan writes are transactional and cancellation rolls back the transaction.

SQLite startup applies ordered, transactional migrations tracked by `PRAGMA user_version`; it validates the expected table shape after migration and rejects databases created by a newer application version. `App` owns the SQLite catalog and HTTP server, rolls back partially initialized infrastructure, and through the Wails `OnShutdown` hook stops in-flight bound calls before closing resources in reverse order. It is constructed only after the single-instance gate in `application.New` has passed, so a losing second launch never opens the catalog or binds a media port; that second process instead raises the running window and exits. Audio + cover art are streamed by the loopback server (`http.ServeContent`, so Range/seek work); each local track gets an absolute `http://127.0.0.1:<port>/media/<id>` URL. Remote URLs used only by the Web Audio analysis probe go through an authenticated `/stream` proxy with private-network blocking, guarded DNS dialing, redirect limits, and connection/header timeouts. A standalone server is used because the Wails asset handler diverges on media/range requests between dev and platforms. macOS needs the loopback ATS exception in `build/darwin/Info*.plist`. The `LocalMusic` provider reaches the Go service through the guarded Wails adapter, maps generated neutral wire DTOs into domain entities, and receives already-resolved local playback URLs. The bound surface is sized by what the frontend actually calls, not by the repository's query surface: reads that `Home` already delivers are not bound a second time, and the scan use case is `PickAndScan` alone — choosing a folder and indexing it are steps of it, not separately drivable operations. Settings triggers scans through the native folder-dialog adapter, which attaches its sheet to the window that made the call — Wails carries that window on the call's context, which is the only window-correct source once more than one exists and the only one that survives the app not being frontmost.
