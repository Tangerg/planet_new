...
import {PlayQueue as PlayQueueModel} from "../../model/playqueue";
...
export class PlayQueue extends Plugin {
    ...
    changeShuffleEnable(): void {
        ...
        const playTracks = this.shuffleEnable ? shuffleArray(this.displayQueue.tracks) : this.displayQueue.tracks
    ...
    autoNext() {
        ...
        if (this.playQueue.isLast &&
            this.repeat.current === RepeatMode.OFF) {
            return
        }
        this.next()
    }
