...
function XmbCategory({ cat, active, dim, onClick }: { cat: any; active: boolean; dim: number; onClick: () => void }) {
    ...
    <div style={{
      ...
      <div style={{ width: 60, display: "grid", placeItems: "center", flex: "0 0 auto", position: "relative", zIndex: 1 }}>
        <div data-art="1" data-xmb-active-art={active ? "1" : undefined} style={{
          ...
          {item.icon && !item.image && (Icon[item.icon] ? React.createElement(Icon[item.icon], { size: active ? 20 : 15, filled: true }) : null)}
        </div>
      </div>
...
export function XMB({ cats, accent, playing, showWaves = true, onOpen, cState, setCState, rowsState, setRowsState }: Props) {
  ...
  const setItem = (n: number) => setRows((r: any) => ({ ...r, [c]: Math.max(0, Math.min(cat.items.length - 1, n)) }));
  ...
  return (
    <div className="fade-in" style={{ position: "absolute", inset: 0, overflow: "hidden", background: "#06060a" }}>
      ...
      <div key={`${c}-${it}`} className="fade-in" style={{
        ...
        filter: "blur(54px) saturate(1.35)", transform: "scale(1.18)", overflow: "hidden",
      }}>
        {item.image && <img src={item.image} alt="" draggable={false}
          style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }}/>}
      </div>
