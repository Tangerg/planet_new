...
export function SearchScreen({ data, onPlay, current, playing, accent, initialQuery = "",
                        openArtist, openAlbum, openPlaylist, liked, toggleLike, search }: SearchScreenProps) {
  ...
  useEffect(() => {
    const term = q.trim();
    if (!term) { setResults({ tracks: [], artists: [], albums: [] }); setLoading(false); return; }
    let alive = true;
    setLoading(true);
    const id = setTimeout(() => {
      const fn = search ?? (async () => ({ tracks: [], artists: [], albums: [] }));
      fn(term).then((r) => { if (alive) { setResults(r); setLoading(false); } }).catch(() => { if (alive) setLoading(false); });
    }, 320);
  ...
  return (
    <div className="fade-in scroll" style={{ height: "100%", background: "radial-gradient(120% 80% at 30% -5%, #16161d, var(--surf-0))" }}>
      <div style={{ padding: "60px 48px 44px" }}>
        ...
        {!q.trim() ? (
          <div style={{ textAlign: "center", padding: 90, color: "var(--tx-3)", fontWeight: 300, fontSize: 22 }}>Search tracks, artists &amp; albums…</div>
        ) : loading ? (
          <div style={{ textAlign: "center", padding: 90, color: "var(--tx-3)", fontWeight: 300, fontSize: 22 }}>Searching “{q}”…</div>
          ...
          <div style={{ marginTop: 40, display: "grid", gridTemplateColumns: "minmax(280px, 0.9fr) 1.1fr", gap: 48 }}>
            ...
            <div>
              ...
              {top && (
                <button onClick={(e) => { const a = e.currentTarget.querySelector(".grain"); const r = (a || e.currentTarget).getBoundingClientRect(); const run = () => openArtist(top); window.__MORPH ? window.__MORPH(r, top.coverSeed, top.gradient, run, top.image) : run(); }} className="grain rise" style={{
                  ...
                  <Art seed={top.coverSeed} grad={top.gradient} image={top.image} style={{ width: 96, height: 96, borderRadius: "50%",
        ...
        {(artists.length > 0) && (
          <section style={{ marginTop: 44 }}>
            ...
            <div className="hrail">
              {artists.map((a: any) => <MediaCard key={a.id} round title={a.name} sub="Artist"
                seed={a.coverSeed} grad={a.gradient} image={a.image} onClick={() => openArtist(a)}/>)}
            </div>
          </section>
        )}
        {(albums.length > 0) && (
          <section style={{ marginTop: 24 }}>
            ...
            <div className="hrail">
              {albums.map((al: any) => <MediaCard key={al.id} title={al.name} sub={al.artist}
                seed={al.coverSeed} grad={al.gradient} image={al.image} onClick={() => openAlbum(al)} onPlay={() => openAlbum(al)}/>)}
            </div>
          </section>
        )}
      </div>
...
function ChartCard({ title, time, seed, grad, image, onClick }: ChartCardProps) {
  const handle = (e: React.MouseEvent<HTMLButtonElement>) => { const r = e.currentTarget.getBoundingClientRect(); window.__MORPH ? window.__MORPH(r, seed, grad, onClick, image) : onClick(); };
  return (
    <button onClick={handle} onMouseEnter={() => window.__AMBIENT?.(seed, grad)}
      onFocus={() => window.__AMBIENT?.(seed, grad)} className="grain" style={{
    ...
    <div className="fade-in scroll" style={{ height: "100%", background: "radial-gradient(120% 90% at 50% 0%, #16161d, var(--surf-0))" }}>
      <div style={{ padding: "60px 40px 40px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
          {((data.charts?.length ? data.charts : MOCK.charts) as any[]).map((c: any, i: number) => <div key={c.id} className="rise" style={{ animationDelay: (i * 0.05) + "s" }}>
            <ChartCard title={c.title} time={c.updatedAt ? "Updated " + c.updatedAt : "Top chart"} seed={c.coverSeed ?? c.seed} grad={c.gradient} image={c.image} onClick={() => onOpenChart(c)}/></div>)}
        </div>
      </div>
    </div>
  );
...
export function CollectionRow({ name, sub, meta, seed, grad, image, round, onOpen, onPlay, item }: CollectionRowProps) {
  ...
  return (
    <div onMouseEnter={() => { setHover(true); window.__AMBIENT?.(seed, grad); }} onMouseLeave={() => setHover(false)} onClick={handle}
      ...
      <div style={{ position: "relative", flex: "0 0 auto" }}>
        <Art className="clrt" seed={seed} grad={grad} image={image} mono={round}
...
export function LibraryScreen({ data, onPlay, current, playing, accent, openPlaylist, openAlbum, openArtist, liked, toggleLike, initialTab, initialView }: LibraryScreenProps) {
  ...
  const coll = tab === "albums" ? (data.albums?.length ? data.albums : MOCK.albums)
    : tab === "artists" ? (data.artists?.length ? data.artists : MOCK.artists)
      : (data.playlists?.length ? data.playlists : MOCK.playlists);
  const openOf = (o: any) => tab === "albums" ? openAlbum(o) : tab === "artists" ? openArtist(o) : openPlaylist(o);
  const tracksOf = (o: any) => tab === "artists" ? tracks.filter((t: any) => t.artistId === o.id) : (o.tracks || []);
  const subOf = (o: any) => tab === "albums" ? o.artist : tab === "artists" ? "" : (o.kind || "Playlist");
  const metaOf = (o: any) => tab === "albums" ? (o.year + " · " + (o.tracks ? o.tracks.length : 0) + " tracks")
    : tab === "artists" ? "" : ((o.tracks ? o.tracks.length : 0) + " tracks");
  const round = tab === "artists";
  const flowItems = coll.map((o: any) => ({ id: "fl-" + o.id, name: o.name, sub: subOf(o), seed: o.coverSeed, grad: o.gradient, image: o.image, obj: o }));
        ...
        {cardTab && view === "flow" && (
          <div style={{ flex: 1, minHeight: 0, margin: "0 -48px" }}>
            <CoverFlow items={flowItems} center={Math.min(flowCenter, flowItems.length - 1)} setCenter={setFlowCenter} accent={accent}
              ...
              onPlay={(it: any) => { const ts = tracksOf(it.obj); ts[0] && onPlay(ts[0]); }}
              tracksFor={(it: any) => tracksOf(it.obj)} onPlayTrack={onPlay}/>
          </div>
        )}
        {cardTab && view === "grid" && (
          <div style={grid}>
            {coll.map((o: any) => <MediaCard key={o.id} round={round} title={o.name} sub={subOf(o)} seed={o.coverSeed} grad={o.gradient} image={o.image} item={o}
              onClick={() => openOf(o)} onPlay={tab === "artists" ? undefined : () => openOf(o)}/>)}
          </div>
        )}
        {cardTab && view === "list" && (
          <div>{coll.map((o: any) => <CollectionRow key={o.id} round={round} name={o.name} sub={subOf(o)} meta={metaOf(o)}
            seed={o.coverSeed} grad={o.gradient} image={o.image} item={o} onOpen={() => openOf(o)}
            onPlay={() => openOf(o)}/>)}</div>
        )}
        {tab === "songs" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", columnGap: 40 }}>
            <div>{tracks.slice(0, half).map((t: any, i: number) => <TrackRow key={t.id} track={t} index={i + 1} onPlay={onPlay}
              current={current} playing={playing} liked={liked} toggleLike={toggleLike} accent={accent}/>)}</div>
            <div>{tracks.slice(half).map((t: any, i: number) => <TrackRow key={t.id} track={t} index={half + i + 1} onPlay={onPlay}
              current={current} playing={playing} liked={liked} toggleLike={toggleLike} accent={accent}/>)}</div>
          </div>
        )}
        </div>
