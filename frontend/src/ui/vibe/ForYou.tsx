...
export function MediaCard({ title, sub, seed, grad, image, round, onClick, onPlay, item }: MediaCardProps) {
  const handle = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!onClick) return;
    const art = e.currentTarget.querySelector(".art");
    const rect = (art || e.currentTarget).getBoundingClientRect();
    if (window.__MORPH) window.__MORPH(rect, seed, grad, onClick, image); else onClick();
  };
  return (
    <div className={"mcard rise" + (round ? " round" : "")} onClick={handle}
      ...
      <Art seed={seed} grad={grad} image={image} className="art" glow={(round ? null : artPair(seed, grad)[1]) as any}>
        {onPlay && (
    ...
    <div className="grain rise" style={{ position: "relative", height: 320, overflow: "hidden",
      ...
      <div style={{ position: "absolute", inset: 0, zIndex: 3, display: "flex", flexDirection: "column",
        ...
        <div style={{ display: "flex", gap: 14, marginTop: 26, alignItems: "center" }}>
          ...
          <button onClick={(e) => { const banner = e.currentTarget.closest("[data-hero]"); const r = (banner || e.currentTarget).getBoundingClientRect(); window.__MORPH ? window.__MORPH(r, playlist.coverSeed, playlist.gradient, onOpen, playlist.image) : onOpen(); }} className="pill-ghost">Open</button>
...
export function ForYouScreen({ data, onPlay, openPlaylist, openAlbum, openArtist, onNav, accent }: ForYouScreenProps) {
  const greeting = (() => {
    ...
    <div className="fade-in scroll" style={{ height: "100%", background: "radial-gradient(120% 80% at 30% -5%, #181922, #0c0c10 55%, #08080b)" }}>
      <div style={{ padding: "60px 56px 50px" }}>
        ...
        <HeroBanner playlist={featured} accent={accent}
          onOpen={() => openPlaylist(featured)} onPlay={() => openPlaylist(featured)}/>
          ...
          {tiles.map((t: any, i: number) => (
            <div key={t.id} className="tile rise" style={{ animationDelay: (i * 0.03) + "s" }}
              ...
              onClick={(e) => {
                ...
                window.__MORPH ? window.__MORPH(rect, t.coverSeed, t.gradient, run, t.image) : run();
              }}>
              <Art seed={t.coverSeed} grad={t.gradient} image={t.image} className="tart"/>
              <span className="tname">{t.name}</span>
              <button className="tfab" aria-label="Open"
                onClick={(e) => { e.stopPropagation(); openTile(t); }}><Icon.play size={15}/></button>
            </div>
          ))}
        </div>
        ...
        <Rail title="Made for you" onAll={() => onNav("library")}>
          {playlists.map((p: any) => (
            <MediaCard key={p.id} title={p.name} sub={p.kind} seed={p.coverSeed} grad={p.gradient} image={p.image} item={p}
              onClick={() => openPlaylist(p)} onPlay={() => openPlaylist(p)}/>
          ))}
        </Rail>

        <Rail title="Recently played" onAll={() => onNav("library")}>
          {albums.map((al: any) => (
            <MediaCard key={al.id} title={al.name} sub={al.artist} seed={al.coverSeed} grad={al.gradient} image={al.image} item={al}
              onClick={() => openAlbum(al)} onPlay={() => openAlbum(al)}/>
          ))}
        </Rail>
        ...
        <Rail title="Your artists" onAll={() => onNav("library")}>
          {artists.map((ar: any) => (
            <MediaCard key={ar.id} round title={ar.name} sub="Artist" seed={ar.coverSeed} grad={ar.gradient} image={ar.image} item={ar}
              onClick={() => openArtist(ar)} onPlay={() => openArtist(ar)}/>
          ))}
        </Rail>
      </div>
    </div>
