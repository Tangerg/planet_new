...
export default function Shell() {
  ...
  /* Global keyboard shortcuts (TanStack Hotkeys) — discrete app shortcuts only;
     directional arrow nav stays in useSpatialNavigation below. Smart defaults:
     Escape/Mod fire even in inputs, single keys are ignored while typing. The
     Mod+Arrow / Mod+L combos force ignoreInputs so they don't clobber in-field text
  ...
  useHotkey(
    "Mod+ArrowUp",
    () => playback.setVolume(Math.min(100, playback.volume + VOLUME_STEP)),
    HK_IGNORE_INPUTS,
  );
  useHotkey(
    "Mod+ArrowDown",
  ...
  useHotkey(
    "Mod+L",
    () => void (playback.current && toggleLike(playback.current.id)),
    HK_IGNORE_INPUTS,
  );
