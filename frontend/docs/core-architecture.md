...
@domain(充血、纯、可独立单测,无 audio / 无事件)
  PlayQueue 聚合 · RepeatMode · Volume 值对象 · 实体(Track/Album/…)+ 按上下文拆分的端口
...
| —(Phase B)               | `provider:changed`          | `ProviderId`(stable machine id)          |
...
- 每个 provider 是独立插件、独立稳定 id，并以 `MUSIC_SOURCE` 注册由实际上下文端口组成的 `MusicSource`。
- **`ProviderRegistry`**(内置插件,= "管理插件的插件"的有用形态):聚合所有 `MusicSource`、持有当前来源、支持按 `ProviderId` 读取/切换，并在切换后 emit `provider:changed`。
- `MediaService` / `PlaybackService` 不再认死插件 id,经注册表取"当前 provider"。UI 的 React Query 按稳定 `ProviderId` 做 key,切换自动隔离缓存。
...
- **接口命名(无 `I` 前缀,按本质二分)**:
  ...
  - **跨层真端口 / 插件对外能力面 → 留接口,按角色命名**:领域端口按用例角色拆分(`CatalogSource` / `PlaybackResolver` / `AuthProvider` / `UserLibrary`);`@core` 插件对外能力面用 `XxxPort`(`ProviderRegistryPort` / `AnalyserPort`,与各自插件类区分)。
...
- [x] **B1 能力注册表**:内核加 typed `Capability<T>` + `provide/resolve/resolveAll`(`CapabilityRegistry`,经 `PluginContext.registry` 注入插件、`Planet.resolve` 暴露)。transport/queue/volume/progress 各发能力 token；音乐来源经 `MUSIC_SOURCE` 注册由实际上下文端口组成的 `MusicSource`。**删除 `Planet.getPlugin`**，生命周期仍由 id-keyed Map 管理。
- [x] **B2 多 provider**:provider 独立稳定 id(`provider:<providerId>`)+ `MUSIC_SOURCE` 注册；`ProviderRegistry` 暴露 `active`/`providers`/`get`/`setActive` 与 `provider:changed`；组合根把来源的 Catalog/Playback/Auth/User Library 端口分别交给对应应用服务。
