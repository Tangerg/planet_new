...
function FlowWaves({ accent }) {
  // a few drifting bezier strokes — the signature XMB "wave"
  const paths = [
    { d: "M-200 380 C 200 240, 520 520, 900 360 S 1500 220, 1800 420", w: 1.6, o: .4, dur: 28 },
    { d: "M-200 440 C 260 360, 560 600, 920 440 S 1520 320, 1800 500", w: 1.1, o: .24, dur: 36 },
    { d: "M-200 320 C 240 460, 600 200, 940 380 S 1480 540, 1800 340", w: .8, o: .15, dur: 46 },
  ];
  return (
    <svg viewBox="0 0 1280 736" preserveAspectRatio="none" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", zIndex: 2, pointerEvents: "none" }}>
      <defs>
      ...
      {paths.map((p, i) => (
        <path key={i} d={p.d} fill="none" stroke="url(#wv)" strokeWidth={p.w} opacity={p.o}
...
function XmbCategory({ cat, active, dim, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      display: "grid", placeItems: "center", width: sz, height: sz,
...
function XmbItem({ item, active, o }) {
  ...
  return (
    <div style={{
      position: "relative",
      display: "flex", alignItems: "center", gap: active ? 22 : 18,
      filter: `blur(${blur}px)`, opacity: op,
      transition: `opacity .55s ${XMB_EASE}, filter .55s ease`,
    }}>
      <div style={{ width: 60, display: "grid", placeItems: "center", flex: "0 0 auto", position: "relative", zIndex: 1 }}>
        <div data-art="1" data-xmb-active-art={active ? "1" : undefined} style={{
          width: iconSz, height: iconSz, borderRadius: active ? 12 : 8, background: artBg(item.seed, item.grad),
          ...
          display: "grid", placeItems: "center", color: "#fff", transition: `all .55s ${XMB_EASE}`,
        }}>{item.icon && (Icon[item.icon] ? React.createElement(Icon[item.icon], { size: active ? 20 : 15, filled: true }) : null)}</div>
      </div>
      <div style={{ minWidth: 0, position: "relative", zIndex: 1 }}>
        <div style={{
          fontSize: active ? 27 : 18, fontWeight: 300, letterSpacing: active ? ".02em" : ".005em", lineHeight: 1.1,
          color: active ? "#fff" : "rgba(255,255,255,.8)",
          textShadow: "0 2px 18px rgba(0,0,0,.6)", transition: `all .55s ${XMB_EASE}`,
          whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 340,
        }}>{item.label}</div>
        {active && <div className="fade-in" style={{ height: 2, width: 42, marginTop: 10, borderRadius: 2,
          background: "var(--accent)", boxShadow: "0 0 12px -1px var(--accent)" }}/>}
        {active && item.sub && <div className="mlabel fade-in" style={{ marginTop: 9, fontSize: 10.5,
          color: "rgba(255,255,255,.55)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 340 }}>{item.sub}</div>}
      </div>
    </div>
  );
}

function XMB({ cats, accent, playing, showWaves = true, onOpen, cState, setCState, rowsState, setRowsState }) {
  const [cI, setCI] = useState(1);          // active category (fallback)
  const [rowsI, setRowsI] = useState({});   // remembered item index per category (fallback)
  ...
  const setItem = (n) => setRows(r => ({ ...r, [c]: Math.max(0, Math.min(cat.items.length - 1, n)) }));
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      else if (e.key === "ArrowUp")   { e.preventDefault(); setItem(it - 1); }
      else if (e.key === "ArrowDown") { e.preventDefault(); setItem(it + 1); }
      else if (e.key === "Enter")     { e.preventDefault();
        if (item) { const node = document.querySelector("[data-xmb-active-art]");
          if (onOpen && node) onOpen(item, node.getBoundingClientRect()); else if (item.run) item.run(); } }
    };
  ...
  return (
    <div className="fade-in" style={{ position: "absolute", inset: 0, overflow: "hidden", background: "#06060a" }}>
      <style>{`
      ...
      <div key={`${c}-${it}`} className="fade-in" style={{
        position: "absolute", inset: "-8%", zIndex: 0, background: artBg(item.seed, item.grad),
        filter: "blur(54px) saturate(1.35)", transform: "scale(1.18)",
      }}/>
      ...
      <div style={{ position: "absolute", left: `calc(${XMB_ANCHOR} - 30px)`, top: XMB_BAR_Y, zIndex: 6 }}>
        {cat.items.map((m, i) => {
          const o = i - it;
          const y = o >= 0 ? XMB_BELOW + o * XMB_ROW + (o >= 1 ? XMB_AFTER_ACTIVE : 0) : -(XMB_ABOVE + (-o - 1) * XMB_ROW);
          ...
          return (
            <div key={m.key} onClick={(e) => (o === 0 ? openItem(m, e) : setItem(i))}
              style={{ position: "absolute", left: 0, top: 0, cursor: "pointer",
                transform: `translate(${cx}px, ${y}px)`, transition: `transform .6s ${XMB_EASE}` }}>
      ...
      <div style={{ position: "absolute", left: XMB_ANCHOR, top: XMB_BAR_Y, zIndex: 9 }}>
        <div style={{ transform: `translateX(${-c * XMB_CAT_GAP}px)`, transition: `transform .62s ${XMB_EASE}` }}>
          {cats.map((cc, i) => {
            ...
            const rot = i === c ? 0 : Math.max(-10, Math.min(10,
              Math.round(Math.atan2(slopeMag, XMB_CAT_GAP) * 57.3 * 0.95)));
            return (
              <div key={cc.id} style={{ position: "absolute", left: i * XMB_CAT_GAP, top: cy,
                transform: `rotate(${rot}deg)`, transition: `top .62s ${XMB_EASE}, transform .62s ${XMB_EASE}` }}>
      ...
      <div style={{ position: "absolute", left: 84, top: 84, zIndex: 8 }}>
        <div style={{ fontSize: 54, fontWeight: 300, color: "#fff", letterSpacing: ".005em", lineHeight: 1,
          whiteSpace: "nowrap", textShadow: "0 2px 24px rgba(0,0,0,.45)" }}>{cat.label}</div>
      </div>

      {/* control hint — bottom-right, clear of the left-aligned item column */}
      <div style={{ position: "absolute", right: 44, bottom: 22, zIndex: 8,
        display: "flex", justifyContent: "flex-end", gap: 26 }}>
        {[["◀ ▶", "Category"], ["▲ ▼", "Browse"], ["↵", "Open"]].map(([k, l]) => (
          <span key={l} style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--accent)" }}>{k}</span>
            <span className="mlabel" style={{ color: "rgba(255,255,255,.45)", fontSize: 10 }}>{l}</span>
          </span>
        ))}
      </div>
    </div>
