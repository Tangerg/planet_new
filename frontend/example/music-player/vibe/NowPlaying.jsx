...
// ============================================================
function LyricLines({ lines, accent, active }) {
  return (
...
function NowPlaying({ track, accent, liked, toggleLike, lyrics, onClose, mono = true, initialMode = "cover", queue = [], onPlay, current, onNext, onPrev }) {
  const [mode, setMode] = useState(initialMode);   // cover | lyrics | comments
  const [queueOpen, setQueueOpen] = useState(false);   // 下轴 = 队列
  const touch = useRef(null);
  const lines = (lyrics && lyrics.length) ? lyrics : [{ line: "No lyrics for this track." }];
  const [active, setActive] = useState(2);
  const [likedC, setLikedC] = useState(new Set());
  const toggleC = (id) => setLikedC(p => { const n = new Set(p); n.has(id) ? n.delete(id) : n.add(id); return n; });
  ...
  return (
    <div className="fade-in" style={{ height: "100%", position: "relative", background: "#08080b", overflow: "hidden" }}
      ...
      <Art seed={coverSeed} grad={grad} mono={mono} slotId="np-hero"
        placeholder="drop a portrait" style={{ position: "absolute", inset: 0, height: "100%" }}>
      ...
      <div className="scroll" style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: "70%", zIndex: 22,
        ...
        <div style={{ padding: "8px 44px 44px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "10px 0 14px", borderBottom: "1px solid rgba(255,255,255,.1)", marginBottom: 8 }}>
            ...
            <Art seed={track?.coverSeed} grad={grad} style={{ width: 44, height: 44, flex: "0 0 auto" }}/>
            <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {queue.map((t, i) => (
            <div key={t.id + i} onClick={() => onPlay && onPlay(t)}
              ...
              <Art seed={t.coverSeed} grad={t.gradient} style={{ width: 40, height: 40, flex: "0 0 auto" }}/>
              <div style={{ flex: 1, minWidth: 0 }}>
